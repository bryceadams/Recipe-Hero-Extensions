 ...
 * @author  Mike Jolley
 */
class RH_Print_Updater {
	private $plugin_name = '';
	private $plugin_file = '';
	private $plugin_slug = '';
	private $api_url     = 'http://recipehero.in/?wc-api=wp_plugin_licencing_update_api';
	...
	public function admin_init() {
		...
		if ( ! empty( $_POST[ $this->plugin_slug . '_licence_key' ] ) ) {
			...
			try {
				...
				$activate_results = json_decode( RH_Print_Updater_Key_API::activate( array(
					'email'          => $email,
	...
	public function deactivate_licence() {
		...
		$reset = RH_Print_Updater_Key_API::deactivate( array(
				'api_product_id' => $this->plugin_slug,
	...
	public function key_notice() {
		...
		?><div class="updated">
			...
			<p><?php printf( '<a href="%s">Please enter your licence key</a> to get updates and support for <strong>%s</strong>.', esc_url( admin_url( 'plugins.php#' . sanitize_title( $this->plugin_slug ) ) ), esc_html( $this->plugin_data['Name'] ) ); ?></p>
			<p><small class="description"><?php printf( 'Lost your key? <a href="%s">Retrieve it here</a>.', esc_url( 'https://www.recipehero.in/lost-licence-key/' ) ); ?></small></p>
		</div><?php
	}
	...
	public function styles() {
		wp_enqueue_style( 'wpjm-updater-styles', plugin_dir_url( __FILE__ ) . '/assets/css/admin.css' );
	}
...
new RH_Print_Updater( plugin_dir_path( __FILE__ ) . '../recipe-hero-print.php' );